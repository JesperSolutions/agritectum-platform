...
describe('respondToOfferPublic', () => {
  beforeAll(() => {
    const app = initializeApp({
      apiKey: 'fake', projectId: 'test', appId: 'test', authDomain: 'test', messagingSenderId: 'test' });
  ...
  it('accepts a public offer', async () => {
    ...
    await setDoc(offerRef, { status: 'pending', publicLink: 'public-token', customerName: 'Anna', title: 'Testrun', totalAmount: 1000, currency: 'SEK', createdBy: 'inspector-uid' });
    ...
    await expect(
      respondToOfferPublic(offerRef.id, 'accept')
