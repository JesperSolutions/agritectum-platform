...
const RoofImageAnnotation: React.FC<RoofImageAnnotationProps> = ({
  ...
  useEffect(() => {
    ...
    const hasChanges =
      validatedLength !== originalLength ||
  ...
  const handlePinIssueLink = (pinId: string, issueId: string) => {
    const updatedPins = validatedPins.map((pin: RoofPinMarker) =>
      pin.id === pinId ? { ...pin, issueId } : pin
    );
    ...
    return (
      <div className={`${className}`}>
        ...
        <div className='border-2 border-dashed border-gray-300 rounded-lg p-6 text-center'>
          ...
          <p className='text-xs text-gray-500 mb-4'>{t('form.messages.addPinsToMarkIssues')}</p>
    ...
    <div className={`${className}`}>
      <div className='flex items-center justify-between mb-2'>
        ...
        <div className='flex gap-2'>
          ...
          >
            {showPins ? t('form.buttons.hidePins') : t('form.buttons.showPins')} (
            {validatedPins.length})
          </button>
          <button
            type='button'
      ...
      <div className='relative border border-gray-300 rounded-lg overflow-hidden bg-gray-100'>
        ...
        <div className='bg-gray-50 border-t border-gray-200 px-4 py-3'>
          <p className='text-xs text-gray-600'>
            {t('roofImage.pinInstruction') ||
              '💡 Klicka på bilden för att lägga till markörer som markerar problemplatser. Markörer kan länkas till specifika problem.'}
          </p>
        </div>
      </div>
