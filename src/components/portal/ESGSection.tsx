...
const ESGSection: React.FC<ESGSectionProps> = ({
  metrics,
  loading = false,
  error = null,
}) => {
            ...
            <div className="p-4 bg-orange-50 border border-orange-200 rounded-lg">
              <p className="text-sm text-gray-700">
                {(t('buildings.esg.recommendation.neutrality') || 
                  'CO₂ neutrality is projected in {years} years. Consider additional improvements to accelerate this timeline.').replace('{years}', metrics.neutralityTimeline.toString())}
              </p>
            </div>
          )}
        </div>
