...
const PublicOfferView: React.FC = () => {
  ...
  const loadOffer = async () => {
    ...
    try {
      ...
      try { if (fetchedOffer?.id) logOfferEvent({ type: 'offer_view', offerId: fetchedOffer.id }); } catch {}
    } catch (err) {
  ...
  const calculateDaysRemaining = () => {
    ...
    const validUntil = (offer as any).validUntil && typeof (offer as any).validUntil?.toDate === 'function'
  ...
  const doAccept = async () => {
    ...
    setProcessing(true);
    try {
      await respondToOfferPublic(offer.id, 'accept', undefined, { name: customerName, email: customerEmail });
      try { if (offer?.id) logOfferEvent({ type: 'offer_confirm_success', offerId: offer.id }); } catch {}
      
      // Update UI immediately - optimistic update
      setOffer(prev => prev ? { ...prev, status: 'accepted' as const, respondedAt: new Date().toISOString() } : null);
  ...
  const handleReject = async () => {
    ...
    try {
      ...
      try { if (offer?.id) logOfferEvent({ type: 'offer_reject', offerId: offer.id }); } catch {}
      
      // Update UI immediately - optimistic update
      setOffer(prev => prev ? { ...prev, status: 'rejected' as const, respondedAt: new Date().toISOString() } : null);
...
{t('common.goToHomepage') || 'Go to Homepage'}
  ...
  const isExpiredOrResponded = isExpired || offer.status === 'accepted' || offer.status === 'rejected';
            ...
            <div className="bg-blue-50 border-l-4 border-blue-500 p-4">
              <div className="flex">
                ...
                <div>
                  ...
                  <p className="text-blue-600 text-sm">Valid until {(((offer as any).validUntil && typeof (offer as any).validUntil?.toDate === 'function') ? (offer as any).validUntil.toDate() : new Date((offer as any).validUntil)).toLocaleDateString('sv-SE')}</p>
                </div>
              </div>
            </div>
          )}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-4">
              We'd appreciate if you could tell us why you're declining this offer. This helps us improve our service.
            </p>
            <textarea
      ...
      {showConfirm && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center p-4 z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-md w-full p-6">
            ...
            <p className="text-gray-600 mb-4">Please confirm your details (optional) and acceptance of this offer.</p>
            ...
            <div className="flex gap-3">
              ...
              <button
                onClick={() => { try { if (offer?.id) logOfferEvent({ type: 'offer_accept_click', offerId: offer.id }); } catch {}; doAccept(); }}
                disabled={processing}
