...
import { Plus, DollarSign, Clock, CheckCircle, XCircle, AlertCircle, FileText } from 'lucide-react';
...
const OffersPage: React.FC = () => {
  ...
  const handleDeleteOffer = async (offerId: string) => {
    ...
    if (
      !confirm(
        t('offers.deleteConfirm', { customerName: offer.customerName }) ||
          `Are you sure you want to delete the offer for ${offer.customerName}?`
      )
    ) {
      return;
    }
          ...
          <div className='bg-white rounded-lg shadow-xl max-w-2xl w-full mx-4 max-h-[80vh] overflow-hidden flex flex-col'>
            <div className='p-6 border-b border-gray-200 flex items-center justify-between'>
              <h2 className='text-xl font-semibold text-gray-900'>
                {t('offers.selectReport') || 'Select a Report'}
              </h2>
              <button
            ...
            <div className='p-6 overflow-y-auto flex-1'>
              {availableReports.length === 0 ? (
                <p className='text-gray-500 text-center py-8'>
                  {t('offers.noReportsAvailable') || 'No reports available'}
                </p>
              ) : (
