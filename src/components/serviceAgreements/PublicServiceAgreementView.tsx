...
import {
  getServiceAgreementByPublicToken,
  acceptServiceAgreementPublic,
...
const PublicServiceAgreementView: React.FC = () => {
  ...
  useDocumentTitle(
    agreement
      ? `${t('serviceAgreement.public.title')} – ${agreement.title}`
      : t('serviceAgreement.public.title')
  );
  ...
  const validateForm = (): boolean => {
    ...
    if (!customerName.trim()) {
      errors.customerName =
        t('serviceAgreement.public.validation.nameRequired') || 'Name is required';
    }

    if (!customerEmail.trim()) {
      errors.customerEmail =
        t('serviceAgreement.public.validation.emailRequired') || 'Email is required';
    ...
    if (!acceptAgreement) {
      errors.acceptAgreement =
        t('serviceAgreement.public.validation.acceptanceRequired') ||
  ...
  const handleAccept = async () => {
    ...
    try {
      ...
      await acceptServiceAgreementPublic(
        agreement.id,
        {
      ...
      setError(
        err.message ||
          t('serviceAgreement.public.acceptError') ||
          'Failed to accept service agreement'
      );
    } finally {
      setProcessing(false);
    }
      ...
      <div className='min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 flex items-center justify-center p-4'>
        <div className='max-w-md w-full bg-white rounded-xl shadow-lg p-8 text-center'>
          ...
          <p className='text-gray-600 mb-6'>
            {error ||
              t('serviceAgreement.public.notFoundMessage') ||
              'The service agreement you are looking for does not exist or has been removed.'}
          </p>
          <button
      ...
      <div className='max-w-4xl mx-auto'>
                      ...
                      <div className='space-y-2 ml-4'>
                        {[
                          ...
                          },
                          {
                            key: 'walkwayControl',
                            label:
                              t('serviceAgreement.form.services.walkwayControl') ||
                              'Kontrol af gangbaner',
                          },
                          {
                            key: 'photoDocumentation',
                            label:
                              t('serviceAgreement.form.services.photoDocumentation') ||
                              'Fotodokumentation',
                          },
                        ]
                  ...
                  {agreement.addons && (
                    <>
                      ...
                      {agreement.addons.sedum && agreement.addons.sedum.length > 0 && (
                        <div>
                          <h4 className='text-sm font-medium text-gray-700 mb-2'>
                            {t('serviceAgreement.form.addons.sedum.title') ||
                              'SEDUMTAG (GRØNT TAG):'}
                          </h4>
                          <div className='space-y-2 ml-4'>
              ...
              <div className='bg-gray-50 rounded-lg p-4 space-y-2'>
                <div className='flex justify-between'>
                  <span className='text-gray-600'>
                    {t('serviceAgreement.public.startDate') || 'Start Date'}:
                  </span>
                  <span className='font-medium'>{formatDate(agreement.startDate)}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-gray-600'>
                    {t('serviceAgreement.public.endDate') || 'End Date'}:
                  </span>
                  <span className='font-medium'>{formatDate(agreement.endDate)}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-gray-600'>
                    {t('serviceAgreement.public.nextService') || 'Next Service Date'}:
                  </span>
                  <span className='font-medium'>{formatDate(agreement.nextServiceDate)}</span>
                </div>
                <div className='flex justify-between'>
                  <span className='text-gray-600'>
                    {t('serviceAgreement.public.frequency') || 'Service Frequency'}:
                  </span>
                ...
                <div className='bg-gray-50 rounded-lg p-4 space-y-3'>
                  ...
                  {agreement.price && (
                    <div className='flex justify-between items-center'>
                      <span className='text-gray-600'>
                        {t('serviceAgreement.public.price') || 'Price'}:
                      </span>
                  ...
                  {agreement.pricingStructure && (
                    <div className='space-y-2 pt-2 border-t border-gray-200'>
                      {(agreement.pricingStructure.perRoof ||
                        agreement.pricingStructure.perRoof === 0) && (
                        <div className='flex justify-between items-center'>
                          <span className='text-gray-600'>
                            {t('serviceAgreement.public.pricingPerRoof') ||
                              'Pris start pr. år per tag:'}
                          </span>
                      ...
                      {(agreement.pricingStructure.perSquareMeter ||
                        agreement.pricingStructure.perSquareMeter === 0) && (
                        <div className='flex justify-between items-center'>
                          <span className='text-gray-600'>
              ...
              <div>
                ...
                <div className='bg-gray-50 rounded-lg p-4'>
                  <p className='text-gray-500 text-sm italic'>
                    {t('serviceAgreement.public.documentsPlaceholder') ||
        ...
        {showSuccess && isAccepted && (
          <div className='bg-white rounded-xl shadow-lg p-8 text-center'>
            ...
            <p className='text-gray-600 mb-6'>
              {t('serviceAgreement.public.successMessage') ||
                'Your service agreement has been accepted. We will contact you shortly.'}
            </p>
          </div>
        )}
      </div>
