...
const RoofForGoodCalculator: React.FC<{ building?: Building }> = ({ building }) => {
          ...
          ? {
              ...
              area:
                state.totalRoofSize > 0
                  ? (state.totalRoofSize * (Math.round(seg.percentage * scaleFactor * 10) / 10)) / 100
                  : 0,
            }
          : seg
      );
  ...
  const handleNext = () => {
    ...
    if (state.step === 2 && !state.location.address) {
      setState((prev) => ({
        ...prev,
        errors: { location: t('esg.calculator.errors.locationRequired') || 'Please enter a location' },
      }));
      return;
    }
  ...
  const renderStep1 = () => (
    <div className="space-y-6">
      ...
      <div className="grid md:grid-cols-3 gap-4">
        {[
          { value: 'homeowner', label: t('esg.calculator.roles.homeowner') || 'Homeowner', icon: '🏠' },
          { value: 'professional', label: t('esg.calculator.roles.professional') || 'Professional', icon: '👔' },
          { value: 'investor', label: t('esg.calculator.roles.investor') || 'Investor', icon: '💼' },
        ].map((role) => (
          <button
            key={role.value}
      ...
      {state.errors.role && <div className="text-red-600 text-sm text-center">{state.errors.role}</div>}
    </div>
  );
        ...
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            ...
            <p className="text-sm text-slate-600">
              {t('esg.calculator.step3.configureAreas') || 'Configure different areas with specific solutions'}
            </p>
          </div>
          ...
          {state.segments.map((segment, idx) => (
            <div key={segment.id} className="bg-white border border-slate-200 rounded-lg p-6">
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    <input
                      ...
                      className="flex-1 px-3 py-2 border border-slate-300 rounded-lg focus:ring-2 focus:ring-blue-500 outline-none"
                    />
                    <span className="flex items-center px-3 bg-slate-100 rounded-lg text-sm font-medium">%</span>
                  </div>
                  <p className="mt-2 text-sm text-slate-600">
              ...
              <div className="grid md:grid-cols-4 gap-4 text-center">
                ...
                <div>
                  <p className="text-lg font-bold text-slate-900">{segment.energy.toLocaleString()}</p>
    ...
    if (!state.metrics) {
      return (
        <div className="text-center py-12">
          ...
          <p className="text-slate-600">{t('esg.calculator.step4.noMetrics') || 'No metrics calculated'}</p>
        </div>
      );
    }
              ...
              <div className="flex items-start justify-between mb-3">
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    <h4 className="font-semibold text-slate-900">{segment.name}</h4>
                    <p className="text-sm text-slate-600">
                      {segment.percentage}% ({segment.area.toLocaleString()} m²) • {segment.solution}
                    </p>
                  </div>
                </div>
                <span className="text-right">
          ...
          <div className="grid md:grid-cols-3 gap-6">
            <div className="text-center">
              ...
              <p className="text-xs text-slate-600 mt-1">
                {t('esg.calculator.step4.co2AbsorptionPerYear') || 'CO₂ absorption per year'} ≈ {totalCO2} kg CO₂/year
              </p>
            </div>
            <div className="text-center">
              <p className="text-4xl font-bold text-blue-600">{getTotalEnergy().toLocaleString()}</p>
              <p className="text-sm font-medium text-slate-900 mt-2">
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100">
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        ...
        <div className="mt-8 flex justify-between items-center">
          ...
          <div className="flex gap-3">
            {state.step === 4 && (
              <button
                onClick={() => showSuccess(t('esg.calculator.reportSaved') || 'Report saved successfully!')}
      ...
      <div className="bg-white border-t border-slate-200 mt-16 py-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 text-center text-sm text-slate-600">
          <p>© 2025 Agritectum - {t('esg.calculator.footer') || 'Sustainable Building Solutions'}</p>
        </div>
      </div>
    </div>
  );
