...
import {
  getESGServiceReportsByBranch,
  deleteESGServiceReport,
...
const ESGReportsList: React.FC = () => {
  ...
  const fetchReports = async () => {
    ...
    try {
      setLoading(true);
      setError(null);
      const branchId = currentUser?.role === 'superadmin' ? currentUser.branchId : currentUser.branchId;
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-slate-50 to-slate-100 font-material">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        {/* Header */}
        <div className="mb-6">
          <h1 className="text-3xl font-bold text-gray-900">{t('admin.esgReports.title') || 'ESG Reports'}</h1>
          <p className="mt-2 text-gray-600">{t('admin.esgReports.subtitle') || 'Manage and view all ESG service reports'}</p>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6">
          <div className="flex flex-col md:flex-row gap-4 justify-between">
            ...
            <div className="relative flex-1 max-w-md">
              ...
              <input
                type="text"
                placeholder={t('admin.esgReports.searchPlaceholder') || 'Search by building, address, or customer...'}
                value={searchTerm}
            ...
            <div className="flex flex-wrap gap-3 items-center">
              ...
              >
                ...
                <option value="public">{t('admin.esgReports.filter.public') || 'Public Only'}</option>
                <option value="private">{t('admin.esgReports.filter.private') || 'Private Only'}</option>
              </select>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-slate-500">{t('admin.esgReports.stats.total') || 'Total Reports'}</p>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-slate-500">{t('admin.esgReports.stats.public') || 'Public Reports'}</p>
                <p className="text-2xl font-bold text-slate-900">{reports.filter(r => r.isPublic).length}</p>
              </div>
            </div>
          </div>
          ...
          <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm text-slate-500">{t('admin.esgReports.stats.totalRoofArea') || 'Total Roof Area'}</p>
        ...
        {filteredReports.length === 0 ? (
          <EmptyState
            ...
            description={debouncedSearchTerm
              ...
              : (t('admin.esgReports.createFirstReport') || 'Create your first ESG report to get started')
            }
            actionLabel={!debouncedSearchTerm ? (t('admin.esgReports.createNew') || 'Create Report') : undefined}
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-3 mb-2">
                      ...
                      <div className="min-w-0 flex-1">
                        <h3 className="font-semibold text-slate-900 truncate">
                          {report.buildingName || t('admin.esgReports.unknownBuilding') || 'Unknown Building'}
                        </h3>
                    ...
                    <div className="flex flex-wrap gap-4 text-sm text-slate-600">
                      ...
                      {report.isPublic ? (
                        <div className="flex items-center gap-1.5">
                          ...
                          <span className="text-green-600">{t('admin.esgReports.public') || 'Public'}</span>
                        </div>
                      ) : (
                        <div className="flex items-center gap-1.5">
                          <XCircle className="w-4 h-4 text-slate-400" />
                          <span className="text-slate-500">{t('admin.esgReports.private') || 'Private'}</span>
                        </div>
                      )}
                    </div>
                    ...
                    {report.calculatedMetrics && (
                      <div className="mt-3 grid grid-cols-2 md:grid-cols-4 gap-3 bg-slate-50 p-3 rounded-lg">
                        <div>
                          <p className="text-xs text-slate-500">{t('admin.esgReports.co2Saved') || 'CO₂ Saved/Year'}</p>
                          <p className="font-semibold text-green-600">
                            {((report.calculatedMetrics.co2ReductionKgPerYear || report.calculatedMetrics.annualCO2Offset || 0) / 1000).toFixed(1)} t
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-slate-500">{t('admin.esgReports.energySaved') || 'Energy Saved/Year'}</p>
                          <p className="font-semibold text-blue-600">
                            {(report.calculatedMetrics.energySavingsKwhPerYear || report.calculatedMetrics.solarPotential || 0).toLocaleString()} kWh
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-slate-500">{t('admin.esgReports.sustainability') || 'Sustainability'}</p>
                          <p className="font-semibold text-purple-600">
                            {report.calculatedMetrics.sustainabilityScore?.toFixed(0) || 0}/100
                          </p>
                        </div>
                        <div>
                          <p className="text-xs text-slate-500">{t('admin.esgReports.rating') || 'Rating'}</p>
