...
export const testReportDeletion = async () => {
  ...
  try {
    ...
    const branchQuery = query(reportsRef, where('branchId', '==', branchId), limit(5));
    ...
    if (!testReport) {
      ...
      try {
        ...
        if (!verifyDoc.exists()) {
          ...
          console.log(
            "💡 The issue was that you were trying to delete temporary reports that don't exist in Firestore"
          );
        } else {
    ...
    if (!reportDoc.exists()) {
      ...
      console.log(
        '🔍 This suggests the report ID exists in the query but not in the actual document'
      );
  ...
  try {
    ...
    const canDelete =
      ...
      (permissionLevel >= 2 || // Superadmin
        ...
        branchId === 'main'); // Branch admin in same branch
