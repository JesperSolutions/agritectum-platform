/**
 * @legacy
 * @movedFrom src/components/AddressWithMap.tsx
 ...
 * - Replaced by AddressWithMapV2.tsx which uses Leaflet.js instead of Google Maps
 * - Not imported or used anywhere in the codebase
 * - AddressWithMapV2 is the canonical implementation
 *
  ...
  const getAddressSuggestions = async (query: string) => {
    ...
    try {
      ...
      if (response.ok) {
        ...
        const suggestions =
          data.predictions?.slice(0, 5).map((prediction: any) => prediction.description) || [];
