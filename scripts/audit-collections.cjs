 ...
 * Run: node scripts/audit-collections.cjs
 */
    ...
    snapshot.docs.forEach((doc, index) => {
      ...
      expectations.required.forEach(field => {
        if (field in data) {
          ...
          const displayValue = typeof value === 'string' ? value.slice(0, 50) : JSON.stringify(value).slice(0, 50);
      ...
      expectations.optional.forEach(field => {
        if (field in data) {
          const value = data[field];
          const displayValue = typeof value === 'string' ? value.slice(0, 50) : JSON.stringify(value).slice(0, 50);
          console.log(`    ℹ️  ${field}: ${typeof data[field]}`);
          if (!fieldPresenceMap[field]) fieldPresenceMap[field] = 0;
          fieldPresenceMap[field]++;
        }
      });
      ...
      Object.keys(data).forEach(field => {
        if (!allExpected.includes(field) && !['id', 'createdAt', 'updatedAt', 'createdBy', 'createdByName'].includes(field)) {
  ...
  try {
    ...
    if (!appointmentSnap.empty) {
      ...
      if (apt.customerId) {
        ...
        console.log(`    Customer: NO ID STORED`);
      }
      ...
      if (apt.buildingId) {
        ...
        console.log(`    Building: NO ID STORED`);
      }
      ...
      if (apt.scheduledVisitId) {
        ...
        console.log(`    ScheduledVisit: NO ID STORED`);
      }
    }
    ...
    if (!visitSnap.empty) {
      ...
      if (visit.appointmentId) {
        ...
        console.log(`    Appointment: NO ID STORED`);
      }
